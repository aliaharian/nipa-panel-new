...
async function invoicesList(page: number, filters: any) {
  const params = Object.keys(filters || {})
    .map((key) => (filters[key] !== null ? `${key}=${filters[key]}&` : ""))
    .join("")
    .slice(0, -1);
...
async function changeFactorStatuses(data: {
  factorId: number;
  statusSlug: string;
  description: string;
  name: string;
...
async function getFactorPayments(id: number) {
  let response = await Api()?.get(`/factor/paymentStep/${id}`);
  return response?.data?.data;
}
...
async function updateFactorItem(factorId: number, values: factorItem) {
  let response = await Api()?.put(
    `/factor/${factorId}/factorItem/${values.id}`,
    {
      code: values.code,
      name: values.name,
      count_type: values.countType,
...
async function deleteFactorItem(factorId: number, itemId: number) {
  let response = await Api()?.delete(
    `/factor/${factorId}/factorItem/${itemId}`
  );
  return response?.data;
}
...
async function addPaymentStep(factorId: number, values: any) {
  let response = await Api()?.post(`/factor/paymentStep`, {
    ...
    allow_online:
      values.isOnline == "online" || values.isOnline == "all" ? true : false,
    allow_offline:
      values.isOnline == "offline" || values.isOnline == "all" ? true : false,
