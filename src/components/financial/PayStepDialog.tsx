                ...
                <div className="p-7 text-[18px]">
                    {data &&
                      ...
                      {transform.toPersianDigitsPutCommas(data.remaining_payable_price.toString())} {t("toman")}
                                ...
                                {(formik.values.payMethod === "offline" && data?.remaining_payable_price > 0) &&
                                    <div className="w-[490px] flex flex-col gap-y-3 mt-6">
                                        <div
                                            className="text-[14px] text-text-500 w-full h-10 px-6 bg-white rounded-[6px] flex items-center justify-between">
                            ...
                            </div>
                            {
                                (formik.values.payMethod === "offline" && data?.remaining_payable_price > 0) &&
                                <div
                                    className="w-full flex svg:text-primary-main text-primary-main text-[14px] font-medium items-start justify-start gap-x-[6px]">
                                    <InfoCircle/>
                                    <p className="leading-[30px]">
                                        لطفا مبلغ کل زیر را به صورت کارت به کارت یا با فیش به حساب های معرفی شده واریز
                                        نمائید و فیش پرداخت خود را در همین بخش آپلود کنید
                                    </p>
                                </div>
                            }
                            {
                                (data?.remaining_payable_price > 0 && formik.values.payMethod === "offline") &&
                    ...
                    ">
                            ...
                            {
                                (formik.values.payMethod === "online" && data?.remaining_payable_price > 0) &&
                                        ...
                                        options={[
                                            {
                                                label:
                                                    <div
                                                        className={`${formik.values.gateway === "saman" ? "" : "grayscale"
                                                        }`}>
                                                        <img src={saman}/>
                                                    </div>,
                                                value: "saman"
                                            },
                                            {
                                                label:
                                                    <div
                                                        className={`${formik.values.gateway === "shaparak" ? "" : "grayscale"
                                                        }`}>
                                                        <img src={shaparak}/>
                                                    </div>,
                                                value: "shaparak"
                ...
                <div className="py-[24px] border-t border-text-300 px-7 flex justify-between">
                    ...
                    <div className="w-[207px]">
                        <Button
                            // disabled={submitdisabled}
                            // icon={<Add />}
                            text={data?.remaining_payable_price > 0 ?
